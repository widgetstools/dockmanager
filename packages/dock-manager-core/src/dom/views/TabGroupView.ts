...
export class TabGroupView {
  ...
  private buildContent(): void {
    // Remove content slots for panels that are no longer in this group.
    // Must happen BEFORE the `existing` early-return below — otherwise a
    // panel that left and later returned would find its stale slot, take
    // the early-return path, and display an empty wrapper because its
    // persistent render container was moved to another group in between.
